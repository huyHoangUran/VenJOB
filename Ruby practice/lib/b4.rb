...
POWERS_HASH = { 10 ** 4  => '万', 10 ** 8 => "億", 10 ** 12 => "兆" , 10 ** 16 => "京", 10 ** 20 => "垓", 10 ** 24 => "𥝱", 10 ** 28 => "穣", 10 ** 32 => "溝", 10 ** 36 => "澗", 10 ** 40 => "正", 10 ** 44 => "載", 10 ** 48 => "極", 10 ** 52 => "恒河沙", 10 ** 56 => "阿僧祇", 10 ** 60 => "那由他", 10 ** 64 => "不可思議", 10 ** 68 => "無量大数", 10 ** 72 => "那由多" }

class Numeric
  def to_kansuji(rs = [])
    return rs.join << NUMS_HASH[0] if self.zero? || self == ""
    
    reverse_array = self.to_s.reverse.split('').each_slice(4).to_a.reverse
    arr_size = reverse_array.size
    
    reverse_array.each do |val|
      next (arr_size -= 1) if val.reverse.join.to_i.zero?
      
      if val.reverse.join.to_i != 0
        rs << val.reverse.join.to_i.group_f_to
        rs << POWERS_HASH[10 ** (4 * (arr_size - 1))]
      end
...
class String
  def to_number result = 0, temp2 = []
    result = 0 if self == "零"
    POWERS_HASH.each { |key, val|  self.sub!(val,'\&,') }
    temp1 = self.split(",")
    temp1.each { |num| temp2 << num.split(",") if POWERS_HASH.each{ |key, val| num.sub!(val,',\&') }}
    return result if temp2.each do |num|
      ac_num = 0
      kan2num = 1
      kan2num = POWERS_HASH.key(num[1]) if num[1]
